...
const EMPTY_FORM: CreatePermissionPayload = { name: "" };

export default function CreatePermissionModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const queryClient = useQueryClient();

  const [form, setForm] = useState<CreatePermissionPayload>(EMPTY_FORM);
  const [errors, setErrors] = useState<Partial<CreatePermissionPayload>>({});
  ...
  const validate = () => {
    const errs: Partial<CreatePermissionPayload> = {};
    if (!form.name.trim()) errs.name = "اسم الصلاحية مطلوب";
    setErrors(errs);
      ...
      >
        <div className="w-full max-w-md rounded-2xl shadow-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/10 shrink-0">
            <h2 className="text-base font-semibold text-slate-800 dark:text-white">إضافة صلاحية جديدة</h2>
            <button
              type="button"
              onClick={onClose}
              disabled={isPending}
          ...
          {/* Form */}
          <form id="createPermissionForm" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
            {apiError && (
            ...
            <Field label="اسم الصلاحية (Key)" error={errors.name}>
              <input
                className={inputCls}
