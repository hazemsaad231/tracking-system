...
import { Search, Moon, Sun, ChevronDown, Calendar, Bell, LogOut, Settings } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  // Close dropdowns when clicking outside could be added, but for now we'll handle blur or explicit close
  const handleLogout = () => {
    logout();
    navigate('/login');
          ...
          {user && (
            <div className="relative">
              <button
                ...
                className="relative w-10 h-10 flex items-center justify-center rounded-xl text-slate-500 hover:text-purple-600 hover:bg-purple-50 dark:hover:text-purple-400 dark:hover:bg-purple-500/10 transition-all duration-200"
          ...
          {user ? (
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-3 pl-1 pr-2 sm:px-3 py-1.5 rounded-full sm:rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 group"
              >
                ...
                <ChevronDown size={14} className={`hidden md:block text-slate-400 group-hover:text-violet-500 transition-transform duration-200 ${showUserMenu ? 'rotate-180' : ''}`} />
              </button>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700/50 py-2 z-50 animate-in fade-in slide-in-from-top-4 duration-200">
                    ...
                    <div className="p-2">
                      <Link 
                        ...
                        className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-violet-600 dark:hover:text-violet-400 transition-colors"
                      >
                        <Settings size={16} />
                        <span>الإعدادات</span>
                      </Link>
